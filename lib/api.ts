...
/**
 * Creates an in-app notification row. The `send-push` edge function is wired
 * to a Database Webhook on INSERT into `notifications`, so this single call
 * both (a) makes the notification show up on the bell screen, and
 * (b) triggers a real push notification if the recipient has a device token.
 */
...
/**
 * Looks up the user_id (owner) of a vendor, given the vendor's id, so we
 * can notify the right person when something happens on their listing.
 */
